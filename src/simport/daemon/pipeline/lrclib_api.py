...
class LRCLibError(Exception):
    """Exception raised for errors in the lrclib.net API."""
...
@dataclass
class Lyrics:
    """Represents a lyrics entry from the lrclib.net API."""
    id: int
    trackName: str
    artistName: str
    albumName: str
    ...
    def to_whisperx_segments(self) -> list[SegmentData]:
        """
...
class LRCLibClient:
    """
    Python Client for the lrclib.net API (https://lrclib.net/api).
    """
    ...
    def get_cached_lyrics(self, track_name: str, artist_name: str, album_name: str, duration: int) -> Lyrics:
        ...
        """
        logger.info(f"LRCLib: Fetching cached lyrics for '{artist_name} - {track_name}'...")
        params = {
            "track_name": track_name,
            "artist_name": artist_name,
            "album_name": album_name,
    ...
    def get_lyrics_by_id(self, lyrics_id: int) -> Lyrics:
        """
        Retrieves a specific lyrics entry by its lrclib.net ID.
        """
        logger.info(f"LRCLib: Fetching lyrics by ID {lyrics_id}...")
    ...
    def search_lyrics(self, 
        ...
        """
        logger.info(f"LRCLib: Searching for lyrics. Params: q='{q}', track='{track_name}', artist='{artist_name}'")
        if not q and not track_name:
            logger.error("LRCLib Search failed: neither 'q' nor 'track_name' provided.")
        ...
        try:
            ...
            if response.status_code >= 500:
                logger.warning(f"LRCLib returned {response.status_code}. Treating as no results.")
                data = []
            else:
                logger.error(f"LRCLib API HTTP error: {e}")
                raise e
        
        result = []
        if isinstance(data, list):
            result = [Lyrics.from_dict(item) for item in data]
            logger.info(f"LRCLib: Found {len(result)} results.")
        else:
            logger.info("LRCLib: No results found (or invalid data format).")
